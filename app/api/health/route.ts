...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!q || typeof q !== 'string') return new Response(JSON.stringify({ error: 'Empty question' }), { status: 400 });
    ...
    const { object, usage } = await generateObject({
      ...
      prompt: [
        ...
        'User input:', q, want_bn ? 'Include a short Bangla bn_summary.' : 'bn_summary=null'
      ].join('\n'),
