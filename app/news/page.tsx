...
type Filters = {
  country?: string; // 'gb', 'us', 'bd', …
  topic?: string;   // 'world', 'sports', 'technology', …
  lang?: string;    // 'en', 'bn', …
  q?: string;
};
...
export default function NewsPage() {
  ...
  // Compose the API URL based on filters
  ...
  // Helper to toggle a pill on/off
  ...
  return (
    <section>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {/* Topic: World */}
        <Pill
          active={(filters.topic ?? 'world') === 'world'}
          onClick={() => toggle('topic', 'world')}
          label="World"
        />
        {/* Country: Bangladesh (bd) */}
        <Pill
          active={filters.country === 'bd'}
          onClick={() => toggle('country', 'bd')}
          label="Bangladesh"
        />
        {/* Topic: Sports */}
        <Pill
          active={filters.topic === 'sports'}
          onClick={() => toggle('topic', 'sports')}
          label="Sports"
        />
        {/* Topic: Technology */}
        <Pill
          active={filters.topic === 'technology'}
          onClick={() => toggle('topic', 'technology')}
          label="Technology"
        />

        {/* Optional: language toggle (English/Bangla) */}
        <Pill
          active={(filters.lang ?? 'en') === 'en'}
          onClick={() => toggle('lang', 'en')}
          label="EN"
        />
        <Pill
          active={filters.lang === 'bn'}
          onClick={() => toggle('lang', 'bn')}
          label="বাংলা"
        />
      </div>
      ...
      <SearchBar
        value={filters.q || ''}
        onChange={(v) => setFilters({ q: v || undefined })}
      />
      ...
      {!loading && !err && items.length === 0 && (
        <div className="text-slate-500">No headlines available for the selected filters.</div>
      )}
