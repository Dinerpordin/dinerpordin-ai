...
export default function HealthPage() {
  ...
  const [wantBn, setWantBn] = useState(false);
  ...
  async function analyze() {
    ...
    try {
      const r = await fetch('/api/health', {
        ...
        body: JSON.stringify({ q, want_bn: wantBn, compact })
      });
  ...
  return (
    <section>
      <h1 className="text-2xl font-semibold mb-2">Healthcheck AI (Educational)</h1>
      <p className="text-sm text-slate-600 mb-4">
        Not medical advice. If symptoms are severe or worsening, seek urgent care.
      </p>
      ...
      <div className="mt-3 flex gap-4 items-center">
        <label className="text-sm">
          <input
            type="checkbox"
            checked={wantBn}
            onChange={(e) => setWantBn(e.target.checked)}
          />{' '}
          Bangla summary
        </label>
        <label className="text-sm">
      ...
      {ui && (
        <section className="mt-6 space-y-4">
          ...
          <Card title="Red flags (seek care urgently)" intent="danger">
            <List items={ui.red_flags} />
          </Card>
          <Card title="Self-care (general info)" intent="success">
            <List items={ui.self_care} />
          </Card>
          {ui.bn_summary ? <Card title="বাংলা সারাংশ">{ui.bn_summary}</Card> : null}
          {ui.usage ? (
            <div className="text-xs text-slate-500">
              Model: {ui.usage.model} • In: {ui.usage.input_tokens ?? '-'} • Out:{' '}
              {ui.usage.output_tokens ?? '-'}
            </div>
          ) : null}
        </section>
      )}
